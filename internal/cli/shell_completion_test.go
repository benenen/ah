...
func TestShellTabCompletion(t *testing.T) {
	...
	for _, shell := range []string{"bash", "zsh"} {
		for _, quoting := range []struct{ name, mark string }{{"unquoted", ""}, {"single", "'"}, {"double", "\""}} {
			t.Run(shell+"/"+quoting.name, func(t *testing.T) {
				...
				destinationName := "destination 中文"
				payload := []byte("Tab completed both independent remote paths\n")
				if err := os.WriteFile(filepath.Join(a.Root, sourceName), payload, 0600); err != nil {
					t.Fatal(err)
				}
				if err := os.Mkdir(filepath.Join(b.Root, destinationName), 0700); err != nil {
					t.Fatal(err)
				}
				...
				write(fmt.Sprintf("ah --config %s --known-hosts %s cp %s B\t~/des\t\n", quote(configPath), quote(known), sourceInput))
				write("printf '\\nAH_RESULT:%s\\n' \"$?\"\n")
				await("\r\nAH_RESULT:")
				...
				got, err := os.ReadFile(filepath.Join(b.Root, destinationName, sourceName))
