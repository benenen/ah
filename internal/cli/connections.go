...
func (a *app) listCommand() *cobra.Command {
	return &cobra.Command{Use: "list", Aliases: []string{"ls"}, Short: "List saved connections", Args: cobra.NoArgs, RunE: func(cmd *cobra.Command, _ []string) error {
...
func (a *app) newCommand() *cobra.Command {
	...
	cmd := &cobra.Command{Use: "new NAME --host HOST --user USER", Aliases: []string{"n"}, Short: "Save a new connection", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
...
func (a *app) editCommand() *cobra.Command {
	...
	cmd := &cobra.Command{Use: "edit NAME [flags]", Aliases: []string{"e"}, Short: "Update only the supplied connection fields", Args: cobra.ExactArgs(1), ValidArgsFunction: a.completeNames, RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err = s.Update(cmd.Context(), func(m map[string]config.Connection) error {
			...
			if cmd.Flags().Changed("sudo-shell") {
				c.SudoShell = changes.SudoShell
			}
			if clearSudoShell {
				c.SudoShell = ""
			}
...
func (a *app) removeCommand() *cobra.Command {
	return &cobra.Command{Use: "remove NAME", Aliases: []string{"rm"}, Short: "Remove a saved connection", Args: cobra.ExactArgs(1), ValidArgsFunction: a.completeNames, RunE: func(cmd *cobra.Command, args []string) error {
		s, err := a.store()
		if err != nil {
			return err
		}
