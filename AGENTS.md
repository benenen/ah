...
ah 使用 Go 对 SSH client 做一层封装，提供 CLI 管理连接；连接配置以 TOML 文件持久化。核心命令包括 `list`、`new`、`rm`、`edit`，以及通过 SFTP 将连接 A 的文件复制到连接 B 的 `cp`。输入源路径和目标路径时支持 Tab 补全，包括远程目录。
...
- 简单优先：围绕上述 CLI 实现，不提前添加服务端、数据库、Web UI 或 LLM。
